# Queries de Controle e gestão 

25 queries para a gestão de custos e auditoria.
Consulte a documentação de [Tablas de Sistema](https://docs.databricks.com/aws/en/admin/system-tables/) para mais detalhes.

Crédito para o projeto de [mousastech](https://github.com/mousastech/monitor/)
Conteúdo original em [Notebook con las queries](https://github.com/mousastech/monitor/blob/main/System%20Tables%20Queries.ipynb)

**01. Qual é o consumo diario do Databricks?**

```sql
SELECT
    DATE(u.usage_date) AS data,
...
**02. Qual o consumo de Databricks por ano/mês?**
...
**03. Qual o consumo de cada SKU no mês atual?**
...
**04. Comparação do consumo do mês atual x mês anterior?**
```sql
SELECT
    after.sku_name,
    before.before_dbus,
...
**05. Quantos usuários ativos mensais há?**
...
**06. Qual o Cluster que mais consome?**
...
SELECT
    c.cluster_name,
    SUM(u.usage_quantity) AS `DBUs Consumido`,
...
**07. Quais os proprietários dos custos interativos?**
...
**08. Quais custoers interaticos os usuários estão acessando?**
...
**09. ¿Que usuários tem o maior tempo de execução dentro de um cluster Interativo?**
...
**10. ¿Quais os comandos que consomem mais tempo dentro dos clusters interativos?**
...
**11. Qual o consumo diário dos notebooks Serverless?**
...
**12. Quanto cada usuário consumiu em Serverless Notebook nos últimos 30 dias?**
...
**13. Quais tarefas do fluxo de trabalho do Databricks consomem mais tempo durante o período?**
...
**14. Qual é o consumo diário do Job Serverless (últimos 60 dias)?**
...
**15. Quanto custa cada diretoria? (Verifique na tag "Diretoria")**

```sql
SELECT u.custom_tags.DIRETORIA as `DIRETORIA`,
...
**16. Quantos tokens são usados ​​em interações com modelos base?**
...
**17. Quais são as mesas mais populares (mais acessíveis)?**
...
**18. Quem mais acessa essas tabelas populares?**
...
FROM 
  system.access.audit
WHERE 
  request_params.table_full_name = '{tabela}'
GROUP BY 
  user_identity.email
ORDER BY 
  qnt_acessos DESC;
...
**19. Quais são as tabelas acessadas por um usuário nas últimas 24 horas?**

```sql
SELECT 
  a.action_name AS `EVENTO`,
  a.event_time AS `CUANDO`,
  IFNULL(a.request_params.full_name_arg, 'Non-specific') AS `TABELA`,
...
**20. Qual é a linhagem de uma determinada tabela?**
...
WHERE 
    source_table_full_name = '{tabela}';
```

**21. Quais entidades lêem de uma determinada tabela?**
...
WHERE 
    tl.source_table_full_name = '{tabela}';
```

**22. Quais tabelas têm comentários vazios?**
...
**23. ¿Quais colunas têm comentários vazios?**
...
**24. Quais são as tabelas menos populares (aquelas que não foram acessadas nos últimos 90 dias)?**
...
**25. Qual comando SQL foi executado em uma determinada tabela?**
...
WHERE l.source_table_full_name = '{tabela}';
```
